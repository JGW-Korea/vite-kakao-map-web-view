...
export default function useKakaoMapController() {
  ...
  // 카카오 지도 초기화 + 웹뷰 통신 컨틀롤러 구성
  useEffect(() => {
    if(!kakaoMapRef.current) return;

    // 웹뷰 통신 이벤트 리스너
    const handleMessage = (event: MessageEvent) => {
      if (typeof event.data !== "string" || !event.data.toString().includes("type") || !kakaoMapRef.current) return; // 모바일 애플리케이션에서 전달받은 구조가 아닌 경우 반환
      ...
      switch(type) {
        case "INIT_MAP": {
          ...
          */
          ...
          kakaoMapInit(map, data.granted, { latitude: data.latitude, longitude: data.longitude });
          break;
        }
        default: {
          break;
    ...
    document.addEventListener("message", handleMessage); // (Androdid) 모바일 <-> 웹뷰 통신 이벤트 리스너 등록
    window.addEventListener("message", handleMessage); // (iOS) 모바일 <-> 웹뷰 통신 이벤트 리스너 등록

    // 컴포넌트 언마운트 시 정돈(clean-up) 작업 수행
    return () => {
